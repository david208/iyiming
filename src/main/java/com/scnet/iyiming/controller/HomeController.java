...
public class HomeController {
	...
	public String login(@RequestParam(value = "errorMsg", required = false) Long errorMsg, Model model) {
		if (null != errorMsg && errorMsg == 1)
			model.addAttribute("errorMsg", "亲爱的用户，您输入的用户名、密码、验证码之一不正确，请重新输入。");
		return "/login";
	}
	 ...
	 * @throws IOException
	 */
	@RequestMapping(value = "/get-validatepic")
	@ResponseBody
	public void getValidatePic(HttpServletRequest request, HttpServletResponse response) throws IOException {
